...
IMPROVED VERSION with better error handling, logging, and CUDA multiprocessing support.
"""
...
def transcribe_segment_on_gpu(
    ...
    # IMPORTANT: Set GPU before any CUDA operations
    ...
    try:
        ...
        # Note: device_index is automatically set to 0 because CUDA_VISIBLE_DEVICES makes this GPU appear as device 0
...
class ParallelWhisperTranscriber:
    ...
    def transcribe_parallel(
        ...
        task: str = "transcribe",
        min_segment_duration: float = 15.0,  # Increased from 10s to 15s
        max_segment_duration: float = 45.0,  # Decreased from 60s to 45s
        ...
        print(f"🚀 Starting parallel transcription on {self.num_gpus} GPUs...")
        ...
        # Use spawn method for ProcessPoolExecutor (CUDA compatible)
        ...
        if failed_segments:
            ...
            for idx, error in failed_segments[:5]:  # Show first 5 errors
